...
const ESTADOS_CONFIG = {
    pendiente: { key: 'pendiente', bg: 'bg-slate-100', text: 'text-slate-700' },
    confirmado: { key: 'confirmado', bg: 'bg-blue-100', text: 'text-blue-700' },
    en_preparacion: { key: 'en_preparacion', bg: 'bg-amber-100', text: 'text-amber-700' },
    en_ruta: { key: 'en_ruta', bg: 'bg-purple-100', text: 'text-purple-700' },
    en_sitio: { key: 'en_sitio', bg: 'bg-indigo-100', text: 'text-indigo-700' },
    en_proceso: { key: 'en_proceso', bg: 'bg-cyan-100', text: 'text-cyan-700' },
    completado: { key: 'completado', bg: 'bg-green-100', text: 'text-green-700' },
    cancelado: { key: 'cancelado', bg: 'bg-red-100', text: 'text-red-700' }
}
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                            ...
                            {esMontaje ? t('operations.assembly') : t('operations.disassembly')}
                        </span>
                        ...
                        {orden.total_equipo > 0 ? (
                            ...
                            !esCompletado && (
                                <span className="flex items-center gap-1 text-amber-600">
                                    ...
                                    {t('operations.dayOrdersModal.noTeam')}
                                </span>
                            )
                        )}
                    </div>
        ...
        <Modal
            ...
            title={
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <span className="block text-lg">{t('operations.dayOrdersModal.ordersOfDay')}</span>
                        {fecha && (
        ...
        >
            ...
            <div className="flex items-center gap-4 mb-4">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg">
                    <span className="text-sm text-slate-600">{t('operations.dayOrdersModal.total')}</span>
                    <span className="text-sm font-bold text-slate-900">{ordenes.length}</span>
                </div>
                {montajes.length > 0 && (
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 rounded-lg">
                        <Package className="w-4 h-4 text-emerald-600" />
                        <span className="text-sm font-medium text-emerald-700">{t('operations.dayOrdersModal.assembliesCount', { count: montajes.length })}</span>
                    </div>
                )}
                {desmontajes.length > 0 && (
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 rounded-lg">
                        <Truck className="w-4 h-4 text-amber-600" />
                        <span className="text-sm font-medium text-amber-700">{t('operations.dayOrdersModal.disassembliesCount', { count: desmontajes.length })}</span>
                    </div>
                )}
            </div>
            ...
            {ordenes.length === 0 ? (
                <div className="py-8 text-center">
                    ...
                    <p className="text-slate-500 font-medium">{t('operations.dayOrdersModal.noOrdersScheduled')}</p>
                    <p className="text-sm text-slate-400">{t('operations.dayOrdersModal.noDayOperations')}</p>
                </div>
            ) : (

...
const TIPO_CONFIG = {
    cancelacion_producto: { key: 'cancellation', icon: Package, color: 'red' },
    solicitud_adicional: { key: 'additionalRequest', icon: Package, color: 'blue' },
    cambio_ubicacion: { key: 'locationChange', icon: MapPin, color: 'purple' },
    dano_elemento: { key: 'damage', icon: Wrench, color: 'orange' },
    otro: { key: 'other', icon: MessageSquare, color: 'slate' }
}

const ESTADO_CONFIG = {
    pendiente: { key: 'pendiente', icon: Clock, classes: 'bg-amber-100 text-amber-700' },
    en_revision: { key: 'inReview', icon: Clock, classes: 'bg-blue-100 text-blue-700' },
    resuelta: { key: 'resolved', icon: CheckCircle, classes: 'bg-green-100 text-green-700' },
    rechazada: { key: 'rejected', icon: X, classes: 'bg-red-100 text-red-700' }
}
  ...
  const { t } = useTranslation()
    ...
    const handleResolver = async (novedadId) => {
        if (!resolucionTexto.trim()) {
            toast.error(t('operations.incidentsList.enterResolution'))
            return
        }
        ...
        try {
            ...
            toast.success(t('operations.incidentsList.incidentResolved'))
            setResolviendoId(null)
            setResolucionTexto('')
        } catch {
            toast.error(t('operations.incidentsList.resolveError'))
        }
    }
        ...
        <div className="space-y-2">
            ...
            {pendientes > 0 && (
                <div className="flex items-center gap-2 text-xs text-amber-700 bg-amber-50 px-3 py-1.5 rounded-lg">
                    ...
                    {t('operations.incidentsList.pendingIncidents', { count: pendientes })}
                </div>
            )}
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                    <span className="text-sm font-medium text-slate-800 truncate">
                                        {t(`operations.incidentsList.${tipo.key}`)}
                                    </span>
                            ...
                            <span className={`text-xs font-medium px-2 py-0.5 rounded-full shrink-0 ${estado.classes}`}>
                                {t(`operations.incidentsList.${estado.key}`)}
                            </span>
                        ...
                        {isExpanded && (
                            <div className="px-3 pb-3 pt-1 border-t border-slate-100 space-y-2">
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500">
                                    <span>{t('operations.incidentsList.reported', { date: formatFechaCorta(novedad.created_at) })}</span>
                                    {novedad.reportada_por_nombre && (
                                        <span>{t('operations.incidentsList.reportedBy', { name: novedad.reportada_por_nombre })}</span>
                                    )}
                                    {novedad.cantidad_afectada > 1 && (
                                        <span>{t('operations.incidentsList.quantityLabel', { count: novedad.cantidad_afectada })}</span>
                                    )}
                                </div>
                                ...
                                {novedad.resolucion && (
                                    <div className="bg-green-50 border border-green-200 rounded-lg p-2.5">
                                        <p className="text-xs font-medium text-green-700 mb-1">{t('operations.incidentsList.resolution')}</p>
                                        <p className="text-sm text-green-800">{novedad.resolucion}</p>
                                        {novedad.resuelta_por_nombre && (
                                            <p className="text-xs text-green-600 mt-1">
                                                {t('operations.incidentsList.resolvedBy', { name: novedad.resuelta_por_nombre, date: formatFechaCorta(novedad.fecha_resolucion) })}
                                            </p>
                                        )}
                                    </div>
                                )}
                                        ...
                                        {resolviendoId === novedad.id ? (
                                            <div className="space-y-2">
                                                <textarea
                                                    ...
                                                    placeholder={t('operations.incidentsList.describeResolution')}
                                                    rows={2}
                                                ...
                                                <div className="flex gap-2">
                                                    ...
                                                    >
                                                        ...
                                                        {t('operations.incidentsList.confirm')}
                                                    </button>
                                                    <button
                                            ...
                                            >
                                                <CheckCircle className="w-3.5 h-3.5" />
                                                {t('operations.incidentsList.resolve')}
                                            </button>
                                        )}
                                    </div>
                                )}

...
class ExportModel {
     ...
     */
    static async obtenerInventarioCompleto(tenantId) {
        const querySeries = `
            SELECT
                e.id AS elemento_id,
            ...
            LEFT JOIN categorias cp ON c.padre_id = cp.id AND cp.tenant_id = s.tenant_id
            LEFT JOIN materiales m ON e.material_id = m.id AND m.tenant_id = s.tenant_id
            LEFT JOIN unidades u ON e.unidad_id = u.id AND u.tenant_id = s.tenant_id
            LEFT JOIN ubicaciones ub ON s.ubicacion_id = ub.id AND ub.tenant_id = s.tenant_id
            WHERE s.tenant_id = ?
        ...
        const queryLotes = `
            ...
            LEFT JOIN categorias cp ON c.padre_id = cp.id AND cp.tenant_id = l.tenant_id
            LEFT JOIN materiales m ON e.material_id = m.id AND m.tenant_id = l.tenant_id
            LEFT JOIN unidades u ON e.unidad_id = u.id AND u.tenant_id = l.tenant_id
            LEFT JOIN ubicaciones ub ON l.ubicacion_id = ub.id AND ub.tenant_id = l.tenant_id
            WHERE l.tenant_id = ?
            GROUP BY e.id, e.nombre, categoria_padre, subcategoria, m.nombre,
     ...
     */
    static async obtenerResumenPorCategoria(tenantId) {
        const query = `
            SELECT
                COALESCE(cp.nombre, c.nombre, 'Sin categoría') AS categoria_padre,
                CASE WHEN cp.nombre IS NOT NULL THEN c.nombre ELSE NULL END AS subcategoria,
                ...
                (
                    ...
                    FROM lotes sub_l
                    INNER JOIN elementos sub_e ON sub_l.elemento_id = sub_e.id AND sub_e.tenant_id = sub_l.tenant_id
                    WHERE sub_e.categoria_id = c.id AND sub_l.tenant_id = ?
                ) +
                (
                    SELECT COUNT(*)
                    FROM series sub_s
                    INNER JOIN elementos sub_e ON sub_s.id_elemento = sub_e.id AND sub_e.tenant_id = sub_s.tenant_id
                    WHERE sub_e.categoria_id = c.id AND sub_s.tenant_id = ?
                ) AS cantidad_total
            FROM elementos e
     ...
     */
    static async obtenerResumenPorUbicacion(tenantId) {
        const query = `
            SELECT
     ...
     */
    static async obtenerAlertasStockBajo(tenantId) {
        const query = `
            SELECT
                e.nombre AS elemento,
            ...
            LEFT JOIN (
                ...
                FROM series
                WHERE estado IN ('bueno') AND tenant_id = ?
